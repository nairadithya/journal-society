...
# ╔═╡ aa979bf4-ff5a-11ea-1163-efbe0cbd8aaf
begin
	import Pkg
	using Flux
	using UnicodePlots
...
# ╔═╡ ce68566e-ff56-11ea-3ff1-bd1feb194b38
md"# MNIST"

# ╔═╡ 4cdb6ec8-ff55-11ea-3939-672bc9f24a98
labels = MNIST.labels(); images = Flux.Data.MNIST.images()

# ╔═╡ 7b2720dc-ff56-11ea-2b98-fff43be1698f
length(labels)

# ╔═╡ 9100a8ec-ff56-11ea-071f-6bbb376910cd
images[1:5]

# ╔═╡ 3ff700be-ff55-11ea-01d6-f7f83382c904
n_inputs = unique(length.(images))[]

# ╔═╡ 4e75ce0c-ff57-11ea-366b-c3d63359a282
n_outputs = length(unique(labels))

# ╔═╡ 5b434fd8-ff57-11ea-2092-6b640b744caa
preprocess(img) = vec(Float64.(img))

# ╔═╡ b3ca8f5e-ff57-11ea-107b-c90e6fe2ca5b
...
# ╔═╡ e2b7de7a-ff57-11ea-0910-97917cae4e54
begin
	trainbatch = create_batch(1:5000)
	@elapsed testbatch = create_batch(5001:6000)
end

# ╔═╡ 6b95c6d8-ff5d-11ea-1327-4b8f5fef72f9
@bind epochs NumberField(0:100, default=10)

# ╔═╡ 44f0a57c-ff58-11ea-032b-6534fb83dbe0
begin
	train_loss = Float64[]
	test_loss = Float64[]		
	...
	model = Chain(
			Dense(n_inputs, n_outputs, identity), softmax
		)
	L(x,y) = Flux.crossentropy(model(x), y)
	opt = Flux.Optimise.Descent()	
	Flux.train!(L, params(model), Iterators.repeated(trainbatch, epochs), opt; cb=Flux.throttle(update_loss!, 1))
end

# ╔═╡ fd19f650-ff5b-11ea-3f41-6bb155606d36
begin
	lineplot(1:length(train_loss), train_loss, title = "train_loss")
	#barplot(1:length(test_loss), test_loss, title = "test_loss")
end

# ╔═╡ 2109d9b2-ff5d-11ea-1016-8b7b630ec84a
test_index = 50001

# ╔═╡ ea8f7900-ff5c-11ea-3852-d1ccfb035122
images[test_index]

# ╔═╡ 17e799b2-ff5d-11ea-163f-b584f2422780
labels[test_index]

# ╔═╡ 17e74658-ff5d-11ea-36d6-371cda74df67
...
manifest_format = "2.0"
project_hash = "068b909813862653512f35817a27405e465cc29b"
